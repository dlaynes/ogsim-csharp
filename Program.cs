...
namespace OgSim
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var rapidfireInfo = new Dictionary<int, Dictionary<int, double>>();
            foreach (KeyValuePair<int, Resource> kvp in resources)
            {
                if (rapidfire.TryGetValue(kvp.Key, out Dictionary<int, int> rf))
                {
                    rapidfireInfo.Add(kvp.Key, kvp.Value.ProcessRapidfires(rf));
                }
            }
            ...
            for (int round = 1; round < turns + 1; round++)
            {
                ...
                attackerGroup.Attack(defenderGroup, rapidfireInfo);
                profiler.StopTask("round_attackers_" + round);
                profiler.StartTask("round_defenders_" + round);
                defenderGroup.Attack(attackerGroup, rapidfireInfo);
                ...
                attackerGroup.AfterRound(round, other: "defenders");
                defenderGroup.AfterRound(round, other: "attackers");
